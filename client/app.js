...
App({
        ...
        if (res.authSetting['scope.userInfo']) {
          ...
          wx.getUserInfo({
            success: function (msg) {
              that._sendUserInfo(msg.userInfo, options);
            }
          })
        }else{
          ...
          that.globalData.showGetUserInfoOptions = options;
        }
      },
      fail: function(res){
        let pageInstance = that.getAppCurrentPage();
        ...
        if (content[i].blockArr[j].actionType === 'custom'){
          ...
          switch (content[i].blockArr[j].action.actionType) {
            ...
            case "call":
              ...
              content[i].blockArr[j].param = '{"phone_num":"' + action['phone-num'] + '"}';
              break;
            case "get-coupon":
  ...
  _userCenterToPage: function (event) {
    ...
    if (openVerifyPhone) {
      if (!this.getUserInfo().phone) {
        this.turnToPage(this.returnSubPackageRouter('bindCellphone') + '?r='+this.getAppCurrentPage().page_router, 1);
      } else {
        if (router === '/promotion/pages/promotionMyPromotion/promotionMyPromotion' || router === 'myPromotion') {
        ...
        if (router === 'myOrder' && goodsType != undefined) {
          ...
          return;
        }else if(router === 'vipCard' && this.globalData.hasFranchiseeList){
      ...
      if (router === 'myOrder' && goodsType != undefined) {
        this.turnToPage(this.returnSubPackageRouter('myOrder') + '?from=userCenterEle&goodsType=' + goodsType + '&currentIndex=' + currentIndex);
        return;
      } else if (router === 'vipCardList' && this.globalData.hasFranchiseeList){
        router = this.returnSubPackageRouter('vipCardList');
      } else {
        router = this.returnSubPackageRouter(router) + '?' + param;
      }
      this.turnToPage(router+'?from=userCenterEle');
    }
  },
  ...
  turnToFranchiseeDetail: function (event) {
    ...
    if(mode == 1){
      this.turnToPage(this.returnSubPackageRouter('franchiseeWaimai') + '?detail=' + appid + shop);
    }else if(mode == 3){
      this.turnToPage(this.returnSubPackageRouter('franchiseeTostore') + '?detail=' + appid + shop);
    }else{
      this.turnToPage(this.returnSubPackageRouter('franchiseeDetail') + '?detail=' + appid + shop);
    }
  },
  ...
  turnToNewsDetail: function (event) {
    let id = event.currentTarget.dataset.id;
    this.turnToPage(this.returnSubPackageRouter('newsDetail') + '?detail=' + id);
  },
...
    return s;
  ...
  globalData:{
    ...
    appTitle: 'FTBN',
    appDescription: 'FTBN',
    appLogo: 'http://img.zhichiwangluo.com/zcimgdir/album/file_5affc67fda7eb.jpg',
